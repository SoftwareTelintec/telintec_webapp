...
const INITIAL_VACATION: Vacation = {
  ...
  seniority: {
    status: "",
    comentarios: "",
    // 'prima' is optional, so it does not need to be included in the initialization
  },
};
...
function VacationsPage() {
  ...
  const getAllVacations = async () => {
    await axios
      .get("http://127.0.0.1:5000/GUI/api/v1/rrhh/employees/vacations/all")
      .then((res) => {
        setRes(res.data);
      })
      .catch((err) => {
  ...
  const postNewVacation = async () => {
    ...
    await axios("http://127.0.0.1:5000/GUI/api/v1/rrhh/employee/vacation", {
      method: "POST",
      data: data,
    })
      .then((res) => {
